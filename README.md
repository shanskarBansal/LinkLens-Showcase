...
```mermaid
flowchart TB
    INPUT["📋 Google Sheet (Input)\nPost Links · State · Page Name"]

    MAIN["🚀 main.py\n(Orchestrator)"]

    READ["📖 read_sheet\n(gspread)"]
    CONFIG["⚙️ params.yaml\n(config)"]
    WRITE["📝 write_sheet\n(formatted)"]

    subgraph ROUTER["🔀 function_influencer.py — URL Platform Router"]
        direction LR
        FB["📘 FB\nEngine"]
        IG["📸 IG\nEngine"]
        YT["🎬 YT\nEngine"]
    ...
    THUMB["🖼️ Thumbnail Engine\n(Playwright / GCS)"]

    MERGE["🔗 pd.concat\n(merge all)"]

    OUTPUT["📊 Google Sheet (Output)\nLikes · Comments · Shares · Engagement · Views\n✨ Orange headers · EB Garamond · Borders"]

    INPUT --> MAIN
    MAIN --> READ & CONFIG
    READ --> ROUTER
    FB & IG & YT & XX --> PARALLEL
    PARALLEL --> THUMB
    THUMB --> MERGE
    MERGE --> WRITE
    WRITE --> OUTPUT
